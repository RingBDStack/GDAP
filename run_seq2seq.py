...
def main():
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(
        ...
        revision=model_args.model_revision,
        use_auth_token=True if model_args.use_auth_token else None,
        # mirror='tuna', # 预训练模型为 uer/t5-base-chinese-cluecorpussmall 时会报错
    )
    ...
    def preprocess_function(examples):
        ...
        inputs = [prefix + inp for inp in inputs] # 在每个句子前面增加 source_prefix 前缀标识
        ...
        if padding == "max_length" and data_args.ignore_pad_token_for_loss:
            labels["input_ids"] = [
                [(l if l != tokenizer.pad_token_id else -100) for l in label] for label in labels["input_ids"] # 用-100来进行填充, -100部分不进行loss的计算
            ]
