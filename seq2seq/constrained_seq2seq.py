...
import torch
torch.set_printoptions(profile="full") # 输出tensor全部内容, 输出到控制台调试时使用
...
class ConstraintSeq2SeqTrainer(Seq2SeqTrainer):
    def __init__(self, decoding_type_schema=None, decoding_format='tree', source_prefix=None, *args, **kwargs):
        ...
        self.decoding_type_schema = decoding_type_schema # 在event任务中为 event schema
    ...
    def prediction_step(
        ...
        def prefix_allowed_tokens_fn(batch_id, sent):
            ...
            # print("input_ids:", inputs.keys())
            # print("src_sentece:", src_sentence)
            # print("sent:", sent)
            # print(self.constraint_decoder.constraint_decoding(src_sentence=src_sentence, tgt_generated=sent))
        ...
        # print("before: ", generated_tokens[0])
        # in case the batch is shorter than max length, the output should be padded
        if generated_tokens.shape[-1] < gen_kwargs["max_length"]:
            generated_tokens = self._pad_tensors_to_max_len(generated_tokens, gen_kwargs["max_length"])
        # print("after: ", generated_tokens[0])
