...
from extraction.predict_parser.target_predict_parser import RolePredictParser, ETPredictParser, TriPredictParser
...
def eval_pred(predict_parser, gold_list, pred_list, text_list=None, raw_list=None):
    ...
    result = dict()
    result.update(trigger_result) # 将trigger_result 添加到result中
    result.update(role_result) # 将role_result 添加到result中
