...
class TaskFormat:
    def generate_sentence(self): pass
...
class DyIEPP(TaskFormat):
    ...
    def generate_sentence(self, type_format='subtype'):
        for sentence, events_in_sentence, sentence_start in zip(self.sentences, self.events, self.sentence_start):
            events = list()
            for event in events_in_sentence: # 一个trigger代表一个事件的形式
                trigger, event_type = event[0]
                trigger -= sentence_start # sentence_start 指的是该句在整个document中的位置

                suptype, subtype = event_type.split('.') # XX.YY 类别信息, XX为大类别, YY为小类别
