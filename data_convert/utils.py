...
def output_schema(event_schema_set, output_file):
    ...
    with codecs.open(output_file, 'w', 'UTF-8') as output:
        output.write(json.dumps(event_type_list, ensure_ascii=False) + '\n')
        output.write(json.dumps(argument_role_list, ensure_ascii=False) + '\n')
        output.write(json.dumps(event_type_list_dict, ensure_ascii=False) + '\n')
